...
def blitImg(sc, tpe, dark, dx, dy):
    if dark == True:
        if tpe == '1':
            img = pygame.image.load(DARK_WALL)
            img_rect = img.get_rect(topleft=(dx, dy))
            sc.blit(img, img_rect)
        else:
            img = pygame.image.load(DARK_GROUND)
            img_rect = img.get_rect(topleft=(dx, dy))
            sc.blit(img, img_rect)
    else:
        if tpe == '0':
    ...
    for i in range(startI, endI):
        for j in range(startJ, endJ):
            if c in sector1 or c in sector2 or c in sector3 or c in sector4 or c in sector5:
                if c in sector1:
                    if sector[i + 1][j] == '1':
                        blitImg(sc, sector[i][j], True, x, y)
                    else:
                        blitImg(sc, sector[i][j], False, x, y)
